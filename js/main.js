...
var download_url = 'https://learnerscrib.platiniumxpwallet.com/materials';
...
$('document').ready(function () {
    ...
    if (localStorage.resource_centre_user_logged_in) {
        ...
        $('#account_management').attr('onclick', "location.assign('./logout/" + myLocation + "')");
    ...
    $('#sort_results_by').on('change', function () {
        ...
        if (sort_by == 'Downloads') {
            Materials_Main.sort((a, b) => (parseInt(a.Download_Count) < parseInt(b.Download_Count) ? 1 : -1))
        }
        if (sort_by == 'Views') {
            Materials_Main.sort((a, b) => (parseInt(a.View_Count) < parseInt(b.View_Count) ? 1 : -1))
        }
        ...
        for (var material_count = 0; material_count < Materials_Main.length; material_count++) {
            ...
            var AddMaterial =
                ...
                '<label>' + parseInt(main_data.View_Count).toLocaleString() + ' Views </label>' +
    ...
    $('#single_file_link_share').on('click', function () {
        var copyText = document.getElementById("single_file_link_share");
    ...
    if (myLocation == 'index') {
        Materials_Url = './php/get_all_materials';
    } else if (myLocation == 'search') {
        Materials_Url = './php/get_searched_materials';
        PassedData = {
            Search_Item: localStorage.resource_centre_search_item
        }
    } else if (myLocation == 'file') {
        Materials_Url = './php/get_current_file';
        PassedData = {
            Current_File: localStorage.resource_centre_current_file
        }
    } else {
        Materials_Url = './php/get_all_materials';
    }
        ...
        success: function (data) {
            ...
            if (data.status == 1) {
                ...
                if (myLocation == 'file') {
                    ...
                    $('.single_file_link_share').val('https://learnerscrib.platiniumxpwallet.com/find_file/' + Materials_Main[0].SN)

                } else {

                    Materials_Main.sort((a, b) => (a.File_Name > b.File_Name ? 1 : -1))
                    for (var material_count = 0; material_count < Materials_Main.length; material_count++) {
                        var main_data_main = Materials_Main[material_count]
                        var AddMaterial =
                            '<a onclick="file_link(' + main_data_main.SN + ')">' +
                            '<div class="main_list_file">' +
                            '<div class="main_list_file_left">' +
                            '<img src="./img_placeholder/' + main_data_main.File_Type + '.png" alt="Material Title (pdf file)">' +
                            ...
                            '</div>' +
                            '<div class="main_list_file_right">' +
                            '<label>' + parseInt(main_data_main.Download_Count).toLocaleString() + ' Downloads </label>' +
                            '<label>' + parseInt(main_data_main.View_Count).toLocaleString() + ' Views </label>' +
                            '<i class="fas fa-star"></i>' +
                            '<i class="fas fa-star"></i>' +
                            '<i class="fas fa-star"></i>' +
                            '<i class="far fa-star"></i>' +
                ...
                //For Top Views
                Materials_Side.sort((a, b) => (parseInt(a.View_Count) < parseInt(b.View_Count) ? 1 : -1))
                for (var material_count_top_views = 0; material_count_top_views < Materials_Side.length; material_count_top_views++) {
                    var main_data_side = Materials_Side[material_count_top_views];
                    var AddMaterial =
                        '<a onclick="file_link(' + main_data_side.SN + ')">' +
                        '<div class="main_right_top_result">' +
                        '<div class="main_right_top_result_left">' +
                        '<img src="./img_placeholder/' + main_data_side.File_Type + '.png">' +
                        '<label> ' + main_data_side.File_Type.toUpperCase() + ' File </label>' +
                        '</div>' +
                        '<div class="main_right_top_result_right">' +
                        '<h3> ' + main_data_side.File_Name + ' </h3>' +
                        '<h6> ' + main_data_side.File_Description.substring(0, 20) + '... </h6>' +
                        '<p> ' + parseInt(main_data_side.View_Count).toLocaleString() + ' Views </p>' +
                        '</div>' +
                        '</div>' +
                        '</a>';
                ...
                //For Top Downloads
                Materials_Side.sort((a, b) => (parseInt(a.Download_Count) < parseInt(b.Download_Count) ? 1 : -1))
                for (var material_count_top_downloads = 0; material_count_top_downloads < Materials_Side.length; material_count_top_downloads++) {
                    var main_data_side = Materials_Side[material_count_top_downloads]
                    var AddMaterial =
                        '<a href="#">' +
                        '<div class="main_right_top_result">' +
                        '<div class="main_right_top_result_left">' +
                        ...
                        '<h3> ' + main_data_side.File_Name + ' </h3>' +
                        '<h6> ' + main_data_side.File_Description.substring(0, 20) + '... </h6>' +
                        '<p> ' + parseInt(main_data_side.Download_Count).toLocaleString() + ' Downloads </p>' +
                        '</div>' +
                        '</div>' +
                        '</a>'
                ...
                $('#main_right_top_results_container_views').html('<h3> No Materials Are Currently Available </h3>')
                $('#main_right_top_results_container_downloads').html('<h3> No Materials Are Currently Available </h3>')
                $('#load_more_button').remove()
            }
        },
        error: function (data) {
